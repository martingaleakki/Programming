...
TimeSeries GetInput(int n)
{
    ...
    for(int i=0;i<n;i++){
        int time=0;
        double price=0;
        cout<<"Enter Time and Price for observation "<<i+1<<endl;
        cin>>time>>price;
        times.push_back(time);
        prices.push_back(price);
    }
    return TimeSeries(times,prices);
}

void TimeSeriesDiffInput()
{
    int n1=0;
    int n2=0;
    cout<<"Enter number of points in time series 1"<<endl;
    cin>>n1;
    TimeSeries ts1 = GetInput(n1);
    cout<<"Enter number of points in time series 2"<<endl;
    cin>>n2;
    TimeSeries ts2 = GetInput(n2);
    TimeSeries tsDiff = TimeSeriesDiff(ts1,ts2);    
    tsDiff.Print();
}
